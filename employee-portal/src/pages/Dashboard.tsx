          ...
          <div className="space-y-3">
            {!noEmployeeRecord && (
              <>
                <Link
                  to="/submit-time"
                  className="flex items-center gap-3 p-3 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-lg transition-colors"
                >
                  <div className="p-2 bg-blue-600 rounded-lg">
                    <Clock className="text-white" size={20} />
                  </div>
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white">Submit Time Entry</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Log your FMLA time</p>
                  </div>
                </Link>
                <Link
                  to="/request-leave"
                  className="flex items-center gap-3 p-3 bg-green-50 dark:bg-green-900/30 hover:bg-green-100 dark:hover:bg-green-900/50 rounded-lg transition-colors"
                >
                  <div className="p-2 bg-green-600 rounded-lg">
                    <Send className="text-white" size={20} />
                  </div>
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white">Request New Leave</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Start a new FMLA request</p>
                  </div>
                </Link>
              </>
            )}
            {isSupervisor && (
              <>
                <Link
                  to="/pending-reviews"
                  className="flex items-center gap-3 p-3 bg-yellow-50 dark:bg-yellow-900/30 hover:bg-yellow-100 dark:hover:bg-yellow-900/50 rounded-lg transition-colors"
                >
                ...
                <Link
                  to="/team-dashboard"
                  className="flex items-center gap-3 p-3 bg-purple-50 dark:bg-purple-900/30 hover:bg-purple-100 dark:hover:bg-purple-900/50 rounded-lg transition-colors"
                >
                  <div className="p-2 bg-purple-600 rounded-lg">
                    <Users className="text-white" size={20} />
                  </div>
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white">Team Dashboard</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">View your team's FMLA status</p>
                  </div>
                </Link>
              </>
            )}
            ...
            <table className="w-full">
              ...
              <tbody>
                {employeeData.recent_submissions.slice(0, 5).map((sub) => (
                  <tr key={sub.id} className="border-b border-gray-100 dark:border-gray-700 last:border-0">
                    <td className="py-3 text-gray-900 dark:text-white">
                      {new Date(sub.leave_date).toLocaleDateString()}
                    </td>
                    <td className="py-3 text-gray-900 dark:text-white">{sub.hours_requested}</td>
                    <td className="py-3">
                      <span
