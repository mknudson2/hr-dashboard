...
export default function ModernDashboard() {
  ...
  const [hiringUpdates, setHiringUpdates] = useState<HiringUpdate[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch hiring updates for hiring managers / stakeholders
        if (features?.is_hiring_manager || features?.is_recruiting_stakeholder) {
          try {
            const data = await apiGet<{ updates: HiringUpdate[] }>(
              '/portal/hiring-manager/hiring-updates'
            );
            setHiringUpdates(data.updates);
          } catch {
            // Hiring updates not available, that's OK
          }
        }
    ...
    fetchData();
  }, [isSupervisor, features?.is_hiring_manager, features?.is_recruiting_stakeholder]);
    ...
    <div className="space-y-8">
      ...
      {hiringUpdates.length > 0 && (
        ...
        >
          ...
          <div className="space-y-3">
            {hiringUpdates.map((update) => (
              <Link
                key={update.id}
                to={update.requisition_id ? `/hiring/requisitions/${update.requisition_id}` : '/hiring/my-requisitions'}
                className="flex items-center justify-between p-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-300 dark:border-gray-700 hover:shadow-lg hover:shadow-gray-200/50 dark:hover:shadow-none transition-all group"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {update.requisition_title}
                      {update.created_at && (
                        <span className="ml-2 text-gray-400 dark:text-gray-500">· {formatRelativeTime(update.created_at)}</span>
                      )}
                    </p>
                  </div>
                </div>
