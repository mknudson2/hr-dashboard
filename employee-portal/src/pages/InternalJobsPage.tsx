    ...
    <div className="p-6">
      {viewMode === 'bifrost' ? (
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-600 mt-1">Browse open positions and apply for internal transfers</p>
        </div>
      )}
      ...
      <p className="text-sm text-gray-500 mb-4">{total} open position{total !== 1 ? 's' : ''}</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white border border-gray-200 rounded-lg">
          <p className="text-gray-500">No internal positions available at this time.</p>
          ...
          {jobs.map(job => (
            ...
            >
              ...
              <div className="flex flex-wrap gap-3 mt-2 text-sm text-gray-500">
                ...
                {job.remote_type && <span className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded text-xs">{job.remote_type}</span>}
                {job.employment_type && <span>{job.employment_type}</span>}
              </div>
              {job.short_description && (
                <p className="text-sm text-gray-600 mt-2 line-clamp-2">{job.short_description}</p>
              )}
              {job.published_at && (
