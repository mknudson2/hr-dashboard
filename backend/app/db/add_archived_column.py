...
def add_archived_column():
    ...
    with engine.connect() as connection:
        ...
        try:
            ...
            """))
            connection.commit()
            logger.info("Added 'archived' column to offboarding_tasks table")
        except Exception as e:
            if "duplicate column name" in str(e).lower():
                logger.warning("'archived' column already exists")
            else:
                raise e


if __name__ == "__main__":
    logger.info("Adding archived column to offboarding_tasks...")
    logger.info("=" * 60)
    add_archived_column()
