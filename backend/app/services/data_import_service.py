            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    if employee:
                        ...
                        # Create new employee using savepoint so a duplicate
                        # constraint error only rolls back this row
                        ...
                        try:
                            ...
                            savepoint.rollback()
                            skipped += 1
                            errors.append(f"Row {idx + 1}: {str(flush_err)}")
                            logger.warning(f"Error inserting row {idx + 1}: {flush_err}")
                            continue
            ...
            for idx, row in enumerate(data_rows):
                try:
                    ...
                    if existing_employee:
                        ...
                        # Create new employee using a savepoint so a duplicate
                        # constraint error only rolls back this row, not the
                        # entire transaction.
                        ...
                        try:
                            ...
                            savepoint.rollback()
                            skipped += 1
                            errors.append(f"Row {idx + 2}: {str(flush_err)}")
                            logger.warning(f"Error inserting row {idx + 2}: {flush_err}")
                            continue
