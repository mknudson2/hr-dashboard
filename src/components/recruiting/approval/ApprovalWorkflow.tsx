...
export default function ApprovalWorkflow({ chainId, request, currentUserId, onActionComplete }: ApprovalWorkflowProps) {
  ...
  const loadChain = async () => {
    try {
      const res = await fetch(`${BASE_URL}/recruiting/approval-chains/${chainId}`, { credentials: 'include' });
  ...
  const handleApprove = async (requestId: number, notes: string) => {
    const res = await fetch(`${BASE_URL}/recruiting/approval-requests/${requestId}/approve`, {
      method: 'POST',
      credentials: 'include',
  ...
  const handleReject = async (requestId: number, notes: string) => {
    const res = await fetch(`${BASE_URL}/recruiting/approval-requests/${requestId}/reject`, {
      method: 'POST',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ notes: notes || undefined }),
      ...
      <div className="space-y-2">
        {chain.steps.map(step => (
          <ApprovalStepCard
            ...
            actedAt={
              request && step.id === request.current_step_id ? request.acted_at : undefined
            }
            notes={
              request && step.id === request.current_step_id ? request.notes : undefined
            }
          />
        ))}
      </div>
